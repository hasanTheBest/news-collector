exports.alokitobangladesh = async function (page) {
  ...
  const articles = await page.evaluate((leadContent) => {
    function getNews(node) {
      ...
      const title =  node.querySelector("h1") ? node.querySelector("h1").innerText.trim() : node.querySelector("h2").innerText.trim() 
